...
function displayBookDetail(bookID, user) {
    displayBookDescription(bookID, user);
    displayPosts(bookID, user);
    filpPage(1, 3);
}

// // display the book information like book cover, author...
function displayBookDescription(bookID, user) {
    ...
    bookCover.src = BooksList[bookID].coverURL;
    ...
    bookName.innerText = 'Name: ' + BooksList[bookID].name

    const bookAuthor = document.createElement('span');
    bookAuthor.className = "bookAuthor";
    bookAuthor.innerText = 'Author: ' + BooksList[bookID].author;
    const bookId = document.createElement('span');
    bookId.className = "bookId";
    bookId.innerText = 'bookID: ' + BooksList[bookID].bookID; 
    const publish = document.createElement('span');
    publish.className = "publish" ;
    publish.innerText = "publish: " + BooksList[bookID].year;
    ...
    const descriContent = document.createTextNode(BooksList[bookID].description)
...
function selectBookToPlay(){
    if (window.location.href.split('?')[1] == null){
        return;
    } else if (window.location.href.split('?')[1].split('&').length === 1){ // guest visit any booklist
        const currentBookID = parseInt(window.location.href.split('?')[1].split('bookID=')[1].split('.')[0])
        // const book = BooksList[currentBookID]
        displayBookDetail(currentBookID, 'guest')
